...
def assign_admin_role(
    ...
    assigned_user = assign_user_role(
        db=db, from_user=current_user, to_user_id=to_user, role=role_to_assign
    )

    if not assigned_user:
        raise HTTPException(
            status_code=404, detail=f"User with id {to_user} not found."
        )
...
def revoke_admin_role(
    ...
    revoked_user = revoke_user_role(
        ...
        to_user_id=to_user,
        role=UserPortalRoles.ROLE_ADMIN,
    )
